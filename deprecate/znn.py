...
def debug():
    rtv = tf.keras.models.Sequential() 
    rtv.add(tf.keras.layers.Conv2D(4,3,data_format="channels_last",activation="relu",padding="same",input_shape=(WIDTH,HEIGHT,CHANNELS)))

    rtv.compile(optimizer='sgd',loss='sparse_categorical_crossentropy',metrics=['acc'])
    return rtv
#创建网络
with strategy.scope():        
    model = ZNN()
...
history = model.fit(get_training_dataset(),  #给出训练数据(已经批处理了，且打乱了)
                        steps_per_epoch=1, #每个epoch所要重复获取数据然后训练的次数
                        epochs=1,         #训练次数
